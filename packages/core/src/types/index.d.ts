...
export type SessionState = 'UNINITIALIZED' | 'DIAGNOSTICS_IN_PROGRESS' | 'DIAGNOSTICS_FAILED' | 'READY_TO_START' | 'STARTING_KIOSK' | 'EXAM_ACTIVE' | 'SESSION_PAUSED' | 'EXAM_TERMINATED' | 'EXAM_COMPLETED' | 'CLEANUP_EXITED';
export type SecurityEventType = 'EXAM_STARTED' | 'EXAM_FINISHED' | 'NAVIGATION_BLOCKED' | 'POPUP_BLOCKED' | 'DOWNLOAD_BLOCKED' | 'PRINT_BLOCKED' | 'CLIPBOARD_BLOCKED' | 'KEYBOARD_HOOK_BLOCKED' | 'DEVTOOLS_BLOCKED' | 'PROCESS_DETECTED' | 'PROCESS_BLOCKED' | 'REMOTE_SESSION_DETECTED' | 'VM_DETECTED' | 'MULTI_MONITOR_DETECTED' | 'INTEGRITY_FAILURE' | 'CONFIGURATION_FAILURE' | 'NETWORK_FAILURE' | 'NETWORK_RECONNECTED' | 'WINDOW_FOCUS_LOST' | 'EXIT_ATTEMPT' | 'UNAUTHORIZED_EXIT' | 'BROWSER_CRASH' | 'SECURITY_POLICY_VIOLATION';
